...
OmniLit 是面向科研文献处理的桌面工具，整合本地账号、多来源文献下载、文献翻译、和远程更新。
...
OmniLit/
  ...
  Workspace/
    ...
    Download/                   下载记录、PDF、预览图和期刊指标 CSV
    Translate/                  翻译 Key、PDF、输出和可写术语表
    Literature/extractions/     文献抽取结果
    task_state/                 后台任务状态
    updates/                    更新临时文件
    ui/avatar-*                 用户头像
  tests/
...
打包资源是只读的。账号库、下载记录、PDF、翻译输出、Key、可写术语表和更新临时文件默认保存在 `Workspace`：

```text
源码运行: 项目根目录\Workspace
Windows: OmniLit.exe 同级目录\Workspace
macOS:   OmniLit.app 同级目录\Workspace
```
...
OmniLit supports filtering download results by a minimum journal metric before PDF resolution and PDF download. The default resolver checks the selected local CSV first, then can fall back to OpenAlex Sources when the local CSV has no match.
