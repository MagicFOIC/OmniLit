...
第二阶段建立可复现、可验证、默认安全失败的桌面与网页发布工程基线，但当前不要求购买或配置
Windows/macOS 平台发行身份。以下事项延期到准备面向公众正式分发桌面安装包时完成：

- 购买或开通 Windows 代码签名服务/证书
- 加入 Apple Developer Program 并创建 Developer ID 证书
- 使用真实发行身份完成 Windows Authenticode 签名
- 使用真实发行身份完成 macOS 签名、公证和票据 stapling
- 配置受保护的 CI 签名环境和生产凭据
- 对已签名、已公证安装包执行 Windows/macOS 真机验收
- 自动发布经过平台签名的更新包

上述延期项不阻塞第二阶段长期目标的基本完成判定，但仍是“正式商业发行桌面安装包”的外部门禁。
在门禁满足前，任何未签名产物都必须明确标记为开发、测试或 `unsigned-smoke`，不得作为正式发行包
自动发布。已有签名/公证脚本和 fail-closed 检查可以保留并测试，但不得要求本阶段提供真实证书、
公证凭据或产生真实签名产物。
...
共享前端不等于共享同一个物理数据目录。数据源必须由受信任的运行环境决定，而不是由页面组件
临时选择：

| 运行环境 | 默认权威数据源 | 身份与隔离 | 本地数据 |
| --- | --- | --- | --- |
| 公网部署的网页端 | Cloud API 与服务器持久化存储 | Cloud API 从登录会话解析用户和 `tenantId`；客户端不得提交或切换 `tenantId` 来决定查询范围 | 不读取桌面 `Workspace`，也不得在 Cloud API 失败时静默回退到 fixture 或 Local Agent |
| Qt 桌面嵌入页 | Local Agent 与桌面 `Workspace` | 本机回环连接、短期凭据和桌面进程边界 | 默认可用并支持离线；登录云端不是读取本地库的前置条件 |
| 独立本地开发网页 | 显式配置的 Local Agent | 仅允许回环地址、精确 Origin 和一次性 Token | 只作为开发/诊断入口，不改变公网网页的云端优先规则 |

因此，网页端登录后看到的文献、集合、图谱、视图、批注、工作区和业务设置，都必须来自服务器中
当前会话所属账户/团队的数据。服务端每一次读写均以认证结果中的 `tenantId` 限定查询；即使请求
路径、查询参数或请求体包含其他租户标识，也不能据此越过租户边界。

桌面端继续保留完整本地访问能力。云同步是显式的跨边界操作，并应保留冲突检测、数据控制和
可见的同步状态；“打开桌面本地数据”与“把本地数据上传到云端”必须是两个不同的权限和用户动作。

生产网页构建一旦配置 Cloud API，业务页面必须在有效云端会话下使用 Cloud API。未登录时应进入
登录流程；未找到云端数据时应显示真实空状态；网络或授权失败时应显示错误。以上情况均不得使用
演示数据伪装成账户数据。
...
- 可复现的 Windows/macOS 构建
- 未签名产物的明确标识和禁止自动正式发布
- 平台签名/公证入口默认安全失败并可在无真实凭据时测试
- 更新清单签名、完整性校验、防回退和替换回滚
- 崩溃收集
- 日志脱敏
- SBOM
- 第三方许可证清单
...
Windows 代码签名、macOS 签名与公证、受保护 CI 发行身份以及签名更新包自动发布，按 2.3 的
延期决策处理，不属于本阶段完成条件。
...
- 自动化构建、测试和可复现产物
- 未签名产物不会被误标或自动发布为正式版本
- 平台签名与公证入口保持 fail-closed，但不要求真实凭据和真实签名产物
- 更新清单签名、完整性校验、防回退和替换回滚可用
- 崩溃监控
- SBOM
- 许可证清单
- 隐私和数据治理
...
购买平台发行身份、实际签名/公证、签名包真机验收和签名更新自动发布是第二阶段后的正式商业
发行门禁，不计入本里程碑完成度。
...
13. 桌面端和网页端具有独立、可靠、可复现的构建与验证流程；未签名桌面产物不会被误作正式版本发布。
14. 商用许可、隐私、安全和依赖得到持续管理。
15. QML 旧图谱已安全退出主开发。
16. Tauri 是否采用可以基于真实数据决定。

基本完成不要求购买 Windows/macOS 发行身份，也不要求产生真实签名或公证产物。面向公众正式分发
桌面安装包之前，仍必须另行完成平台签名、公证、受保护凭据、签名包真机验收和签名更新发布门禁。
