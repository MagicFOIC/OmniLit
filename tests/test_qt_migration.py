...
class QtOnlyTests(unittest.TestCase):
    ...
    def test_qml_uses_shared_responsive_layout_metrics(self) -> None:
        ...
        self.assertIn("literatureLibraryController.previewFor", literature)
        self.assertIn("literatureLibraryController.thumbnailStateFor", literature)
        self.assertIn("ScrollBar.vertical", literature)
        self.assertIn("selectedRecordId", literature)
        self.assertIn("PDF首页预览正在生成", literature)
        self.assertIn("PDF首页预览生成失败", literature)
        self.assertIn("id: previewPopup", literature)
        self.assertIn("点击放大", literature)
        self.assertIn("root.adjustPreviewZoom", literature)
