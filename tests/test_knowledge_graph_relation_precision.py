...
class KnowledgeGraphRelationPrecisionTests(unittest.TestCase):
    def test_same_sentence_with_cue_is_retained(self) -> None:
        relations = relations_for({"pages": [{"page": 1, "textBlocks": [
            ...
            {"blockNo": 2, "text": "We use the AtlasNet model on the OpenImages dataset.", "bbox": [1, 2, 3, 4]},
        ]}]})
        pair = [item for item in relations if item.relation_type == "USES_DATASET" and item.relation_method == "same_sentence"]
        self.assertTrue(pair)
        self.assertTrue(any(item.source_id.startswith("model:") for item in pair))
    ...
    def test_same_block_with_evidence_is_retained(self) -> None:
        ...
        pair = [item for item in relations if item.relation_type == "USES_DATASET" and item.relation_method == "same_block"]
        self.assertTrue(pair)
    ...
    def test_same_section_without_cue_is_not_linked(self) -> None:
        ...
        self.assertFalse(any(item.relation_type == "USES_DATASET" and item.relation_method == "same_section" for item in relations))
    ...
    def test_relation_evidence_and_direction_reason_are_required(self) -> None:
        ...
        semantic = [item for item in relations if item.relation_type == "EVALUATED_BY" and item.source_id.startswith("result:")]
