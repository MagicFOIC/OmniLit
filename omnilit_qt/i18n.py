...
TEXTS: dict[str, tuple[str, str]] = {
    "pdf_parse_mode_fast": ("快速解析：PyMuPDF", "Fast parse: PyMuPDF"),
    "pdf_parse_mode_paddleocr_vl": ("高精度解析：PaddleOCR-VL", "High accuracy parse: PaddleOCR-VL"),
    "pdf_parse_mode_mineru": ("深度解析：MinerU", "Deep parse: MinerU"),
    "pdf_export_markdown": ("导出 Markdown", "Export Markdown"),
    "pdf_export_raw_outputs": ("导出原始解析结果目录", "Export raw parser outputs"),
    "pdf_no_bbox_note": ("该元素无可定位坐标，仅在解析结果中展示。", "This element has no locatable coordinates and is shown only in parsed results."),
    "pdf_engine_errors": ("解析引擎错误", "Parser engine errors"),
    "parser_cloud_services": ("PDF 云解析服务", "PDF cloud parsing services"),
    "parser_cloud_services_detail": ("令牌使用系统加密保存；环境变量配置优先。PDF 将上传到所选服务进行识别。", "Tokens are stored with system encryption and environment variables take precedence. PDFs are uploaded to the selected service for recognition."),
    "service_enabled": ("启用服务", "Enable service"),
    "api_token": ("API 令牌", "API token"),
    "token_saved_placeholder": ("已安全保存；留空可保持不变", "Securely saved; leave blank to keep it"),
    "test_connection": ("测试配置", "Test configuration"),
    "clear_token": ("清除令牌", "Clear token"),
    "save": ("保存", "Save"),
    "sort": ("排序", "Sort"),
    ...
    "nav_research_workspace": ("研究工作空间", "Research workspace"),
    ...
    "system_settings_detail": ("工作目录、登录后提示等全局设置", "Workspace folder, sign-in guide, and global settings"),
    "login_prompt_settings": ("登录后提示管理", "Sign-in guide"),
    "login_prompt_settings_detail": ("管理工作目录，以及每次进入工作区后是否显示功能提示。", "Manage the workspace folder and whether the guide appears after sign-in."),
    "workspace_folder": ("工作目录", "Workspace folder"),
    "save_workspace_folder": ("保存目录", "Save folder"),
    "workspace_folder_saved": ("工作目录已保存。", "Workspace folder saved."),
    "show_guide_every_open": ("每次打开/登录后显示功能提示", "Show the guide every time after opening/signing in"),
    "restart_onboarding": ("立即重新查看功能提示", "Show the guide now"),
    ...
    "graph_style_academic": ("学术流程", "Academic"),
    "graph_style_radial": ("环绕关系", "Radial"),
    "graph_style_focus": ("焦点邻域", "Focus"),
    "graph_density_compact": ("精简", "Compact"),
    "graph_density_normal": ("标准", "Normal"),
    "graph_density_detailed": ("详细", "Detailed"),
    "graph_density_all": ("全部", "All"),
    "graph_depth_full": ("全图", "Full"),
    "graph_depth_one_hop": ("一跳", "1-hop"),
    "graph_depth_two_hop": ("二跳", "2-hop"),
    "graph_depth_tooltip": ("聚焦所选节点的邻居", "Focus neighbors of the selected node"),
    "graph_review": ("待审核", "Review"),
    "graph_fit": ("适配全图", "Fit graph"),
    "graph_fullscreen": ("全屏", "Fullscreen"),
    "graph_settings": ("图谱设置", "Graph settings"),
    "graph_reset": ("重置", "Reset"),
    "graph_display": ("显示", "Display"),
    "graph_arrows": ("箭头", "Arrows"),
    "graph_labels": ("标签", "Labels"),
    "graph_dim_unrelated": ("淡化无关节点", "Dim unrelated"),
    "graph_text_fade_threshold": ("文字显示阈值", "Text fade threshold"),
    "graph_node_size": ("节点大小", "Node size"),
    "graph_link_thickness": ("连线粗细", "Link thickness"),
    "graph_layout": ("布局", "Layout"),
    "graph_animate": ("动画", "Animate"),
    "graph_empty_review": ("没有待审核节点", "No review nodes"),
    ...
    "contact_email_settings": ("联系邮箱", "Contact email"),
    "contact_email_settings_detail": ("用于 OpenAlex、Unpaywall 等文献接口的礼貌联系邮箱。默认使用注册账号，可在这里修改。", "Used as the polite contact email for literature APIs such as OpenAlex and Unpaywall. Defaults to the registered account and can be changed here."),
    "contact_email_saved": ("联系邮箱已保存。", "Contact email saved."),
    "contact_email_invalid": ("请输入有效的联系邮箱。", "Enter a valid contact email."),
    ...
    "filter_quality_preset": ("筛选条件", "Filter condition"),
    "quality_keyword": ("关键词提及", "Keyword mention"),
    "quality_relaxed": ("宽松", "Relaxed"),
    "quality_balanced": ("均衡", "Balanced"),
    "quality_strict": ("严格", "Strict"),
    "quality_very_strict": ("极严格", "Very strict"),
    "quality_keyword_tip": ("尽量多找。只要文献标题或摘要提到你的关键词就会保留，适合第一次摸清领域。", "Find as much as possible. Keeps papers that mention your keyword in the title or abstract. Good for a first scan of a field."),
    "quality_relaxed_tip": ("多收一些相关文献，同时过滤掉明显不沾边的结果。适合刚开始建库。", "Keeps a broad set of related papers while removing clearly unrelated results. Good when starting a library."),
    "quality_balanced_tip": ("推荐默认。数量和相关性比较均衡，适合日常下载和整理。", "Recommended default. Balances result count and relevance. Good for everyday downloading and organizing."),
    "quality_strict_tip": ("少而更准。只保留和关键词关系更明确的文献，适合减少后续人工筛选。", "Fewer, more focused results. Keeps papers clearly related to your keywords. Good when you want less manual cleanup."),
    "quality_very_strict_tip": ("最少但最集中。只保留高度相关、来源更可靠的开放获取文献，适合精读前挑选。", "Smallest and most focused set. Keeps highly related open-access papers from stronger sources. Good before deep reading."),
    "keyword_input_placeholder": ("输入研究主题、材料体系、方法或缩写", "Enter a research topic, material system, method, or abbreviation"),
    "add_keyword": ("添加", "Add"),
    "pdf_backfill": ("PDF 补全", "PDF backfill"),
    "pdf_backfill_tip": ("扫描已有元数据，并下载缺失的合法开放获取 PDF。", "Scan existing metadata and download missing legal OA PDFs."),
    "source_api_settings": ("数据源 API 设置", "Source API settings"),
    "source_api_settings_tip": ("Key 使用 PBKDF2 + Fernet 加密保存，不会写入下载表单配置。", "Keys are stored with PBKDF2 + Fernet encryption and are not written to the download form config."),
    "source_api_expand": ("展开 API 设置", "Show API settings"),
    "source_api_collapse": ("收起 API 设置", "Hide API settings"),
    "source_api_settings_saved": ("数据源 API 设置已保存。", "Source API settings saved."),
    "source_api_settings_failed": ("API 设置保存失败：{error}", "Could not save API settings: {error}"),
    "source_api_key_cleared": ("API Key 已清除。", "API key cleared."),
    "source_api_key_clear_failed": ("该来源没有可清除的 Key。", "This source has no key to clear."),
    "source_api_test_success": ("API 连接测试成功。", "API connection test succeeded."),
    "source_api_test_failed": ("API 连接测试失败。", "API connection test failed."),
    "source_api_not_configured": ("未配置", "Not configured"),
    "source_api_configured": ("已配置", "Configured"),
    "source_api_test_success_short": ("测试成功", "Test passed"),
    "source_api_test_failed_short": ("测试失败", "Test failed"),
    "openalex_api_key": ("OpenAlex API Key", "OpenAlex API Key"),
    "crossref_mailto": ("Crossref 联系邮箱", "Crossref contact email"),
    "europe_pmc_email": ("Europe PMC 联系邮箱", "Europe PMC contact email"),
    "doaj_api_key": ("DOAJ API Key（可选）", "DOAJ API Key (optional)"),
    "semantic_scholar_api_key": ("Semantic Scholar API Key（可选）", "Semantic Scholar API Key (optional)"),
    "api_key_saved_placeholder": ("已安全保存；留空保持不变", "Securely saved; leave blank to keep it"),
    "api_key_optional_placeholder": ("可选，需要时输入新 Key", "Optional; enter a new key when needed"),
    "save_api_settings": ("保存 API 设置", "Save API settings"),
    "clear_key": ("清除 Key", "Clear key"),
    ...
    "pdf_backfill_task": ("补全已有文献 PDF", "Backfill existing PDFs"),
    "pdf_backfill_started": ("已启动已有元数据 PDF 补全。", "Existing metadata PDF backfill started."),
    "pdf_backfill_done": ("PDF 补全完成。", "PDF backfill finished."),
    "pdf_backfill_stopped": ("PDF 补全已停止。", "PDF backfill stopped."),
    "pdf_backfill_failed": ("PDF 补全失败：{error}", "PDF backfill failed: {error}"),
    ...
    "translation_service": ("翻译服务", "Translation service"),
    "omnilit_default_service": ("OmniLit 默认服务", "OmniLit default service"),
    "model_label": ("模型", "Model"),
    "recommended_model": ("推荐模型", "Recommended model"),
    "custom_service": ("使用自己的模型 / API Key", "Use my own model / API key"),
    "custom_service_hint": ("默认服务已就绪；只有需要自己的模型或额度时再开启。", "The default service is ready; enable this only for your own model or quota."),
    "default_service_ready": ("默认服务已就绪，不需要配置 API Key。", "The default service is ready. No API key setup is needed."),
    "default_service_unavailable": ("默认服务暂不可用，可使用自定义 API Key。{error}", "The default service is temporarily unavailable. You can use a custom API key. {error}"),
    "default_service_unavailable_short": ("默认服务暂不可用，可使用自定义 API Key。", "The default service is temporarily unavailable. You can use a custom API key."),
    "more_options": ("更多选项", "More options"),
    "translation_range": ("翻译范围", "Translation range"),
    "range_full": ("全文翻译", "Full translation"),
    "range_quick": ("快速试译前 3 页", "Quick trial: first 3 pages"),
    "range_custom": ("自定义页数", "Custom page count"),
    "first_n_pages": ("前 N 页", "First N pages"),
    "terms_count": ("{count} 个术语", "{count} terms"),
    ...
    "api_key_required": ("默认服务暂不可用，请切换到自定义服务并填写 API Key，或启用仅测试版式。", "The default service is temporarily unavailable. Switch to a custom service and enter an API key, or enable layout-only mode."),
...
TEXTS.update({
    ...
    "onboarding_workdir_dirs": (
        "保存后会自动创建 config / data / cache / runtime / reports 子目录。",
        "OmniLit will create config / data / cache / runtime / reports automatically.",
    ),
    ...
    "onboarding_account_body": (
        "从这里进入账号、语言、外观、更新和系统设置。",
        "Open account, language, appearance, update management, and system settings from here.",
    ),
    "onboarding_language_title": ("界面语言", "Interface language"),
    "onboarding_language_body": (
        "先在这里切换界面语言；后面的外观、更新和系统设置也都从拓展栏进入。",
        "Switch the interface language here first; appearance, updates, and system settings live in the same extension drawer.",
    ),
    ...
    "onboarding_system_settings_title": ("系统设置", "System settings"),
    "onboarding_system_settings_body": (
        "这里可以管理工作目录，以及是否每次打开或登录后重新显示功能提示。",
        "Manage the workspace folder and whether the guide appears again after opening or signing in.",
    ),
...
RU_TEXTS: dict[str, str] = {
    ...
    "nav_library": "Библиотека литературы", "nav_translate": "Перевод литературы", "nav_research_workspace": "Исследовательское пространство", "nav_update": "Обновления", "sidebar_expand": "Развернуть боковую панель",
...
RU_TEXTS.update({
    ...
    "graph_style_academic": "Академический",
    "graph_style_radial": "Радиальный",
    "graph_style_focus": "Фокус",
    "graph_density_compact": "Компактно",
    "graph_density_normal": "Стандартно",
    "graph_density_detailed": "Подробно",
    "graph_density_all": "Все",
    "graph_depth_full": "Весь граф",
    "graph_depth_one_hop": "1 шаг",
    "graph_depth_two_hop": "2 шага",
    "graph_depth_tooltip": "Показать соседей выбранного узла",
    "graph_review": "Проверка",
    "graph_fit": "Вписать граф",
    "graph_fullscreen": "На весь экран",
    "graph_settings": "Настройки графа",
    "graph_reset": "Сброс",
    "graph_display": "Отображение",
    "graph_arrows": "Стрелки",
    "graph_labels": "Подписи",
    "graph_dim_unrelated": "Затемнять несвязанные",
    "graph_text_fade_threshold": "Порог показа текста",
    "graph_node_size": "Размер узлов",
    "graph_link_thickness": "Толщина связей",
    "graph_layout": "Макет",
    "graph_animate": "Анимация",
    "graph_empty_review": "Нет узлов для проверки",
...
RU_TEXTS.update({
    "contact_email_settings": "Contact email",
    "contact_email_settings_detail": "Used as the polite contact email for literature APIs such as OpenAlex and Unpaywall. Defaults to the registered account and can be changed here.",
    "contact_email_saved": "Contact email saved.",
    "contact_email_invalid": "Enter a valid contact email.",
    "filter_quality_preset": "Filter condition",
    "quality_keyword": "Keyword mention",
    "quality_relaxed": "Relaxed",
    "quality_balanced": "Balanced",
    "quality_strict": "Strict",
    "quality_very_strict": "Very strict",
    "quality_keyword_tip": "Find as much as possible. Keeps papers that mention your keyword in the title or abstract. Good for a first scan of a field.",
    "quality_relaxed_tip": "Keeps a broad set of related papers while removing clearly unrelated results. Good when starting a library.",
    "quality_balanced_tip": "Recommended default. Balances result count and relevance. Good for everyday downloading and organizing.",
    "quality_strict_tip": "Fewer, more focused results. Keeps papers clearly related to your keywords. Good when you want less manual cleanup.",
    "quality_very_strict_tip": "Smallest and most focused set. Keeps highly related open-access papers from stronger sources. Good before deep reading.",
    "source_api_settings": "Source API settings",
    "source_api_settings_tip": "Keys are stored with PBKDF2 + Fernet encryption and are not written to the download form config.",
    "source_api_expand": "Show API settings",
    "source_api_collapse": "Hide API settings",
    "source_api_settings_saved": "Source API settings saved.",
    "source_api_settings_failed": "Could not save API settings: {error}",
    "source_api_key_cleared": "API key cleared.",
    "source_api_key_clear_failed": "This source has no key to clear.",
    "source_api_test_success": "API connection test succeeded.",
    "source_api_test_failed": "API connection test failed.",
    "source_api_not_configured": "Not configured",
    "source_api_configured": "Configured",
    "source_api_test_success_short": "Test passed",
    "source_api_test_failed_short": "Test failed",
    "openalex_api_key": "OpenAlex API Key",
    "crossref_mailto": "Crossref contact email",
    "europe_pmc_email": "Europe PMC contact email",
    "doaj_api_key": "DOAJ API Key (optional)",
    "semantic_scholar_api_key": "Semantic Scholar API Key (optional)",
    "api_key_saved_placeholder": "Securely saved; leave blank to keep it",
    "api_key_optional_placeholder": "Optional; enter a new key when needed",
    "save_api_settings": "Save API settings",
    "clear_key": "Clear key",
    "keyword_input_placeholder": "Enter a research topic, material system, method, or abbreviation",
    "add_keyword": "Add",
    "pdf_backfill": "PDF backfill",
    "pdf_backfill_tip": "Scan existing metadata and download missing legal OA PDFs.",
    "pdf_backfill_task": "Backfill existing PDFs",
    "pdf_backfill_started": "Existing metadata PDF backfill started.",
    "pdf_backfill_done": "PDF backfill finished.",
    "pdf_backfill_stopped": "PDF backfill stopped.",
    "pdf_backfill_failed": "PDF backfill failed: {error}",
    "system_settings": "System settings",
    "system_settings_detail": "Workspace folder, sign-in guide, and global settings",
    "login_prompt_settings": "Sign-in guide",
    "login_prompt_settings_detail": "Manage the workspace folder and whether the guide appears after sign-in.",
    "workspace_folder": "Workspace folder",
    "save_workspace_folder": "Save folder",
    "workspace_folder_saved": "Workspace folder saved.",
    "show_guide_every_open": "Show the guide every time after opening/signing in",
    "restart_onboarding": "Show the guide now",
    ...
    "onboarding_workdir_dirs": "OmniLit will create config / data / cache / runtime / reports automatically.",
    ...
    "onboarding_account_body": "Open account, language, appearance, update management, and system settings from here.",
    "onboarding_language_title": "Interface language",
    "onboarding_language_body": "Switch the interface language here first; appearance, updates, and system settings live in the same extension drawer.",
    ...
    "onboarding_system_settings_title": "System settings",
    "onboarding_system_settings_body": "Manage the workspace folder and whether the guide appears again after opening or signing in.",
    ...
    "pdf_parse_mode_fast": "Fast parse: PyMuPDF",
    "pdf_parse_mode_paddleocr_vl": "High accuracy parse: PaddleOCR-VL",
    "pdf_parse_mode_mineru": "Deep parse: MinerU",
    "pdf_export_markdown": "Export Markdown",
...
RU_TEXTS.update({
    ...
    "omnilit_default_service": "OmniLit default service",
    "model_label": "Model",
    "recommended_model": "Recommended model",
    "custom_service": "Use my own model / API key",
    "custom_service_hint": "The default service is ready; enable this only for your own model or quota.",
    "default_service_ready": "The default service is ready. No API key setup is needed.",
    "default_service_unavailable": "The default service is temporarily unavailable. You can use a custom API key. {error}",
    "default_service_unavailable_short": "The default service is temporarily unavailable. You can use a custom API key.",
    "more_options": "More options",
    "translation_range": "Translation range",
    "range_full": "Full translation",
    "range_quick": "Quick trial: first 3 pages",
